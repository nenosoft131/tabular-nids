...
class RunPipeline():
    def __init__(self,
                 ...
                 n_samples_upper_bound: int=120000):
        '''
        ...
        self.seed_list = list(np.arange(2, 6))
    ...
    def model_fit(self,model_name,params):
        ...
        gym_search_space = gym(self=None, mode=model_name, max_combinations=50)
        for config in gym_search_space:
            ...
            try:
                ...
                # self.clf = self.clf.fit(X_train=self.data['X_train'], y_train=self.data['y_train'], 
                #                        n_clusters=config['n_clusters'], contamination=config['contamination'],
                #                        clustering_estimator=config['clustering_estimator'], alpha=config['alpha'], beta=config['beta'])
