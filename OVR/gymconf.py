...
def gym(self, mode='small', max_combinations=10):
    if mode == 'large':
        ...
        gyms = {}  
        gyms['n_estimators'] = [100, 200]
        gyms['class_weight'] = [None, 'balanced'] 
        gyms['max_depth'] = [None, 10]  
        gyms['min_samples_split'] = [2, 10]           # Minimum samples required to split a node
        gyms['min_samples_leaf'] = [1]           # Minimum samples required at a leaf node
        gyms['max_features'] = ['sqrt', 'log2'] # Number of features to consider for best split
        gyms['ccp_alpha'] = [0.0, 0.01] 
    
    elif mode == 'XGB':
        gyms = {} 
        gyms['learning_rate'] = [0.05, 0.1]                  # Impact of each tree on the outcome
        gyms['max_depth'] = [3, 6]                             # Maximum depth of each tree
        gyms['n_estimators'] = [100, 500] 
        gyms['reg_alpha'] = [0, 0.1]                         # L1 regularization (sparsity control)
        gyms['reg_lambda'] = [0.5, 1] 
        
    elif mode == 'PCA':
        gyms = {}
        gyms['contamination'] = [0.1, 0.25, 0.5]
        gyms['svd_solver'] = ['auto', 'full', 'arpack', 'randomized']
        gyms['n_components'] =[1, 10, 15]
