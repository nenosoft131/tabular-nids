...
class RunPipeline():
    def __init__(self,
        ...
        self.seed_list = list(np.arange(1, 2))
        ...
        # fully-supervised algorithms
            ...
            # from sklearn
            # for _ in ['LR','SVM', 'RF', 'XGB']:
            for _ in ['LR','RF','XGB', 'SVM']:
    ...
    def model_fit(self,model_name,params):
        self.score_train = None
        # gym_search_space = gym(self=None, mode='large', max_combinations=120)
        gym_search_space = gym(self=None, mode=model_name, max_combinations=30)
        for config in gym_search_space:
            ...
            try:
                ...
                #                         estimators=config['n_estimators'], max_samples=config['max_samples'],
                ...
                if self.score_train is None:
                    print(f"Calculating Score Train for {self.model_name}")
                ...
                # performance
                # result = self.utils.metric(y_true=self.data['y_test'], y_score=test_probs, val_true=self.data['y_val'], val_score=val_probs, pos_label=1)
                ...
                # get_logger(__name__).info(f"Model: {self.model_name}, AUC-ROC-Test: {result['aucroc']}, AUC-PR-Test: {result['aucpr']}, AUC-ROC-Val: {result['val_aucroc']}, AUC-PR-Val: {result['val_aucpr']}")
                # del self.clf
                gc.collect()
                # df_AUCROC = pd.DataFrame({'model_name': [model_name], 'params': [params], 'config': [config], 'AUCROC-Val': [result] })
                # df_AUCROC = pd.DataFrame({'model_name': [model_name], 'params': [params], 'config': [config], 'AUCROC-Val': [result['val_aucroc']], 'AUCPR-Val': [result['val_aucpr']],'AUCROC-Test': [result['aucroc']], 'AUCPR-Test': [result['aucpr']], 'TIME_FIT' :[time_fit], 'TIME_INFER':[time_inference]})
                print("OUTPUT")
        ...
        return time_fit, time_inference
    ...
    def run(self):
        ...
        for i, params in tqdm(enumerate(experiment_params)):
            ...
            for model_name in tqdm(self.model_dict.keys()):
                ...
                # fit model
                time_fit, time_inference = self.model_fit(model_name, params)
