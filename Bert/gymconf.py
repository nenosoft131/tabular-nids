...
def gym(self, mode='small', max_combinations=10):
    if mode == 'large':
        ...
        gyms = {}
        gyms['learning_rate'] = [2e-5, 3e-5, 5e-5]                     
        gyms['per_device_train_batch_size'] = [16, 32]                
        gyms['per_device_eval_batch_size'] = [16]                     
        gyms['num_train_epochs'] = [3, 4, 5]                          
        gyms['weight_decay'] = [0.0, 0.01]                            
        gyms['max_seq_length'] = [128, 256]                           
        gyms['lr_scheduler_type'] = ['linear', 'cosine']              
        gyms['gradient_accumulation_steps'] = [1, 2]                  
        gyms['seed'] = [42, 123, 2024]                               
        gyms['fp16'] = [False]        
